...
struct ChatService {
    ...
    static func create(from message: Message, with chat: Chat, completion: @escaping (Chat?) -> Void) {
        ...
        // 6
        for uid in chat.memberUIDs {
            multiUpdateValue["chats/\(uid)/\(String(chatRef.key!))"] = chatDict
        }
        ...
        // 8
        multiUpdateValue["messages/\(String(chatRef.key!))/\(String(messageKey!))"] = message.dictValue
    ...
    static func sendMessage(_ message: Message, for chat: Chat, success: ((Bool) -> Void)? = nil) {
        ...
        let messageKey = messagesRef.key
        multiUpdateValue["messages/\(chatKey)/\(String(messageKey!))"] = message.dictValue
